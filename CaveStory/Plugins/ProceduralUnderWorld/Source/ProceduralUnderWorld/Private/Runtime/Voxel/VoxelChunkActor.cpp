...
{
	const int32 LODCount = MaxLODLevel + 1;
	const int32 BaseSize = ChunkSize; // LOD0 기준 해상도
	const int32 BaseNumPoints = (BaseSize + 1) * (BaseSize + 1) * (BaseSize + 1);

	// ✅ LOD0 기준 Scalar Field 먼저 생성
	TArray<float> BaseScalarField;
	BaseScalarField.SetNum(BaseNumPoints);
	GenerateScalarField(BaseScalarField, BaseSize);
	...
	for (int32 LOD = 0; LOD < LODCount; ++LOD)
	{
		...
		const int32 NumVoxels = LODSize + 1;
		const int32 NumPoints = NumVoxels * NumVoxels * NumVoxels;

		TArray<float> ScalarField;
		// ✅ 다운샘플링해서 생성
		GenerateScalarFieldFromBase(ScalarField, BaseScalarField, LODSize, BaseSize);
		...
		ProcMeshComp->CreateMeshSection(LOD, Vertices, Indices, Normals, UVs, Colors, Tangents, false);
		ProcMeshComp->SetMeshSectionVisible(LOD, (LOD == 0));
	}
